...
const SKELETON_STYLES = `
    ...
    /* Glass card for real cards wrapper */
    .glass-card-outer {
        background: linear-gradient(
            135deg,
...
// ─────────────────────────────────────────────────────────────────────────────
//  GLASS SQUARE SKELETON CARD
// ─────────────────────────────────────────────────────────────────────────────
const GlassSquareSkeleton = ({ delay = 0 }: { delay?: number }) => (
    <div
        className="sk-wrap relative overflow-hidden p-5 md:p-6"
                ...
                <div className="flex flex-wrap gap-2">
                    {[72, 58, 66, 50, 70, 54].map((w, i) => (
                        <div
                            key={i}
                            className="sk-badge"
                            style={{ width: w, animationDelay: `${delay + i * 0.08}s` }}
                        />
                    ))}
                </div>
                ...
                <div className="flex flex-wrap gap-3">
                    {[110, 130, 94].map((w, i) => (
                        <div
                            key={i}
                            className="sk-btn"
                            style={{ width: w, animationDelay: `${delay + i * 0.1}s` }}
                        />
                    ))}
                </div>
            </div>
    ...
    useEffect(() => {
        ...
        const boot = async () => {
            try {
                ...
                // Lenis not installed — native scroll fallback
            }
        };
        ...
        <section ref={sectionRef} className="py-24 bg-transparent" id="projects-section">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div ref={headerRef} className="text-center mb-16">
                    <div className="header-pill" style={{ opacity: 0 }}>
                        ✦ Portfolio
                    </div>
                    <h2
                    ...
                    <p
                        className="header-sub text-lg text-white/60 max-w-xl mx-auto"
                        style={{ opacity: 0 }}
                    >
                ...
                {!loading && hasMore && (
                    <div ref={btnRef} className="text-center mt-14" style={{ opacity: 0 }}>
                        <button
                            onClick={loadMore}
                            className="relative px-9 py-3.5 text-sm font-semibold tracking-widest uppercase
                                       text-white rounded-2xl overflow-hidden transition-transform duration-200
